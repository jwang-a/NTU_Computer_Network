...
###utils
def random_drop_relay(sock,segment,msg,sender_ip,sender_port,reciever_ip,reciever_port,TTol,Drop):
    if msg[1][0]==sender_ip and msg[1][1]==sender_port:
        ...
        if segment[2]==1:
            ...
            send_msg(sock,msg[0],reciever_ip,reciever_port)
            exit()
        printlog('get','ack','#'+str(segment[0]))
        send_msg(sock,msg[0],sender_ip,sender_port)
        printlog('fwd','ack','#'+str(segment[0]))
        return TTol,Drop
    else:
        return TTol,Drop
...
def main():
    ...
    while True:
        ...
        TTol, Drop = random_drop_relay(sock,segment,msg,sender_ip,sender_port,reciever_ip,reciever_port,TTol,Drop)
